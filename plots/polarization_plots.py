...
def Sz_rapidity_polarization(polarization_file, mass=1.115683, rap_cut=1, pt_cut=10, vorticity=False, global_integrals = False):
    ...
    if dimy>1:
        ...
        mean_spin = np.trapz(np.trapz(Pz*pt_vals,x=np.unique(pT),axis=0),x=np.unique(y_rap),axis=1)
        spectra = np.trapz(np.trapz(dNdP*pt_vals,x=np.unique(pT),axis=0),x=np.unique(y_rap),axis=1)
    else:
        print("Midrapidity only!")
        Pz = Pizu.reshape((dimP,dimPhi))
        dNdP = dndp.reshape((dimP,dimPhi))
        Pt = pT.reshape((dimP,dimPhi))
        mean_spin = np.trapz(Pz*pt_vals,x=np.unique(pT),axis=0)
        spectra = np.trapz(dNdP*pt_vals,x=np.unique(pT),axis=0)
...
def Sz_pseudorapidity_polarization(polarization_file, mass=1.32171, eta_cut=1, pt_cut=10, vorticity=False, global_integrals = False):
    ...
    if dimeta>1:
        ...
        mean_spin = np.trapz(np.trapz(Pz*jac,x=np.unique(pT),axis=0),x=np.unique(eta),axis=1)
        spectra = np.trapz(np.trapz(dNdP*jac,x=np.unique(pT),axis=0),x=np.unique(eta),axis=1)
    else:
...
def Sz_polarization_pT_pseudo(polarization_file, mass=1.32171, harmonics=2, eta_cut=1, vorticity=False, global_integrals = False):
    ...
    if dimeta>1:
        ...
        jac = ((pt_vals**2 * np.cosh(eta_vals))/(np.sqrt(mass**2 + pt_vals**2 * np.cosh(eta_vals)**2)))
        mean_spin = np.trapz(np.trapz(Pz_reahsped*jac,x=np.unique(phi),axis=1),x=np.unique(eta),axis=1)
        spectra = np.trapz(np.trapz(dNdP_reshaped*jac,x=np.unique(phi),axis=1),x=np.unique(eta),axis=1)
    else:
        print("Midrapidity only!")
        Pzsin = Pizu*np.sin(harmonics*phi) 
        Pz_reahsped = Pzsin.reshape((dimP,dimPhi))
...
def Sj_rapidity_polarization(polarization_file, mass=1.115683, rap_cut=1, pt_cut=10, vorticity=False, global_integrals = False):
    ...
    if dimy>1:
        ...
        dNdP = dndp.reshape((dimP,dimPhi,dimy))
        Pt = pT.reshape((dimP,dimPhi,dimy))
        mean_spin = np.trapz(np.trapz(Py*pt_vals,x=np.unique(pT),axis=0),x=np.unique(y_rap),axis=1)
        spectra = np.trapz(np.trapz(dNdP*pt_vals,x=np.unique(pT),axis=0),x=np.unique(y_rap),axis=1)
    else:
        print("Midrapidity only!")
        ...
        dNdP = dndp.reshape((dimP,dimPhi))
        Pt = pT.reshape((dimP,dimPhi))
        mean_spin = np.trapz(Py*pt_vals,x=np.unique(pT),axis=0)
        spectra = np.trapz(dNdP*pt_vals,x=np.unique(pT),axis=0)
